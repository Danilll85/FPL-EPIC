const getDays = (date: Date, delimiter : string) => {
  let days = "";

  for (let i = 2, j = 1; j < 32; i++, j++) {
    days += `\t${date.getDate()} `;
...
export const drawCalendar = (year : number, month : number | string) => {
  ...
  if (typeof month == "string") {
    month = months.findIndex((value) => value.toLowerCase() == (month as string).toLowerCase());
